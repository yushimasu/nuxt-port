  ...
  <div class="page_articles">
    <header class="header">
      <h1 class="header-title">
        <NuxtLink :to="`/`">
          I'm Web developer<br />in Nagoya
        </NuxtLink>
      </h1>
    </header>
    <main class="section_container">
      <article class="page_article_detail">
        ...
        <div class="article_category">
          <span class="category_label" :class='category.id'>{{ category.category }}</span>
        </div>
        <p class="article_date">{{ $dayjs(publishedAt).locale('ja').format('YYYY年MM月DD日') }}</p>
